            ...
            <script>
                ...
                $(document).ready(function () {
                    var table = $('#example').DataTable({
                        ajax: "jobGson.fin?param=${jobBean.expImp}",
                        "serverSide": true,
                        "bAutoWidth": false,
                        "bJQueryUI": true,
                        "bProcessing": true,
                        ...
                        "aoColumns": [
                            ...
                            },
                            {
                                "mData": "jobNumber",
                                "render": function (mData, full, row) {
                                    return '<a href="jobRetrieve.fin?param=${jobBean.expImp}&jobNumber=' + mData + '">'+mData+'</a>';
                                }
                            },
                            {"mData": "jobDate"},
                            {"mData": "pol"},
                    ...
                    $("div.top").append('&nbsp;&nbsp;&nbsp;&nbsp;' +
                            '&nbsp;&nbsp;<input type="button" id="btnnew" value="New" class="finbutton"/>&nbsp&nbsp;<rbac:rbac pattern="(.)\\|(Y)\\|(Y)\\|(Y)\\|(.)\\|(.)\\|(.)" formId="1049"><input type="button" id="deleteBtn" value="Delete" class="finbutton"/></rbac:rbac>&nbsp;');
                ...
                $(function () {
                    $("#btnnew").on('click', function () {
                        $('#blForm').attr("action", "jobCreate.fin?param=${jobBean.expImp}");
                        $("#blForm").submit();
                    });
                    $("#searchBtn").on('click', function () {
                        $('#blForm').attr("action", "job.fin?param=${jobBean.expImp}");
                        $("#blForm").submit();
                    });
                    $("#deleteBtn").on('click', function () {
                        $('#blForm').attr("action", "deleteJob.fin?param=${jobBean.expImp}");
                        $("#blForm").submit();
                    });
                });
